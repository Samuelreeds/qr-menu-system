...
export default function SuperAdminClient({ shops, invites, users, plans = [], shopStats }: { shops: any[], invites: any[], users: any[], plans?: any[], shopStats?: any }) {
  ...
  return (
    <div className="flex h-screen bg-[#F9FAFB] font-sans overflow-hidden relative">
      ...
      {/* SIDEBAR */}
      <aside className={`w-[260px] border-r border-gray-200 bg-white flex flex-col z-40 fixed md:static h-full transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <nav className="p-4 space-y-2">
          <button 
            onClick={() => { setActiveTab('shops'); setEditingPlanId(null); setIsSidebarOpen(false); }} 
          ...
          <button 
            onClick={() => { setActiveTab('plans'); setEditingPlanId(null); setIsSidebarOpen(false); }} 
          ...
          <button 
            onClick={() => { setActiveTab('invites'); setEditingPlanId(null); setIsSidebarOpen(false); }} 
          ...
          <button 
            onClick={() => { setActiveTab('users'); setEditingPlanId(null); setIsSidebarOpen(false); }} 
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden w-full relative">
        <header className="flex items-center justify-between p-4 md:p-6 bg-white border-b border-gray-100 gap-2 md:gap-4">
          <div className="flex items-center gap-2 md:gap-4 shrink-0">
            <button onClick={() => setIsSidebarOpen(true)} className="md:hidden p-1.5 -ml-1.5 text-gray-600 hover:text-gray-900 transition-colors">
              <Menu size={22}/>
            </button>
            <h1 className="text-lg md:text-xl font-bold capitalize leading-tight">
          ...
          <div className="flex items-center gap-2 md:gap-4 shrink-0">
            ...
            {activeTab === 'invites' && (
              <button 
                ...
                className="hidden md:flex items-center gap-1.5 md:gap-2 bg-gray-900 text-white px-3 py-2 md:px-4 md:py-2 rounded-xl text-xs md:text-sm font-bold hover:bg-black transition-colors whitespace-nowrap"
              >
                <Plus size={16} className="shrink-0 md:w-[18px] md:h-[18px]"/> Create Invite
              </button>
            )}
            {activeTab === 'plans' && !editingPlanId && (
              <button onClick={() => setEditingPlanId('new')} className="hidden md:flex items-center gap-1.5 md:gap-2 bg-gray-900 text-white px-3 py-2 md:px-4 md:py-2 rounded-xl text-xs md:text-sm font-bold hover:bg-black transition-colors whitespace-nowrap">
                <Plus size={16} className="shrink-0 md:w-[18px] md:h-[18px]"/> Create Plan
              </button>
            )}
            {activeTab === 'users' && (
              <button onClick={() => setIsSuperAdminModalOpen(true)} className="hidden md:flex items-center gap-1.5 md:gap-2 bg-gray-900 text-white px-3 py-2 md:px-4 md:py-2 rounded-xl text-xs md:text-sm font-bold hover:bg-black transition-colors whitespace-nowrap">
                <Plus size={16} className="shrink-0 md:w-[18px] md:h-[18px]"/> Create SuperAdmin
              </button>
            )}
            <button className="flex items-center justify-center w-8 h-8 md:w-9 md:h-9 bg-white border rounded-full text-gray-400 hover:text-gray-900 transition-colors shrink-0">
              <Bell className="w-4 h-4 md:w-[18px] md:h-[18px]"/>
            </button>
          </div>
        </header>

        <div className="p-4 md:p-6 overflow-y-auto pb-24 md:pb-6">
        ...
        <div className="md:hidden fixed bottom-6 right-6 z-40">
          {activeTab === 'invites' && (
            <button onClick={() => setIsInviteModalOpen(true)} className="w-14 h-14 bg-gray-900 text-white rounded-full flex items-center justify-center shadow-2xl active:scale-95 transition-transform" title="Create Invite">
              <Plus size={24} />
            </button>
          )}
          {activeTab === 'plans' && !editingPlanId && (
            <button onClick={() => setEditingPlanId('new')} className="w-14 h-14 bg-gray-900 text-white rounded-full flex items-center justify-center shadow-2xl active:scale-95 transition-transform" title="Create Plan">
              <Plus size={24} />
            </button>
          )}
          {activeTab === 'users' && (
            <button onClick={() => setIsSuperAdminModalOpen(true)} className="w-14 h-14 bg-gray-900 text-white rounded-full flex items-center justify-center shadow-2xl active:scale-95 transition-transform" title="Create SuperAdmin">
              <Plus size={24} />
            </button>
          )}
        </div>
