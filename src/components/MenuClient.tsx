    ...
    >
      ...
      <header className="relative overflow-hidden min-h-[160px]" style={{ background: themeColor }}>
        ...
        <div className="relative z-10 flex flex-col items-center justify-center pt-12 pb-8 px-4">
          <div className="absolute top-4 left-0 right-0 px-4 flex justify-between items-center w-full max-w-7xl mx-auto">
              <button 
                onClick={() => setIsInfoOpen(true)} 
                className="p-2.5 rounded-xl bg-white/20 backdrop-blur-md border border-white/30 text-white active:scale-95 flex-shrink-0"
              >
                <Menu size={20} />
              </button>
          ...
          <div className="rounded-full overflow-hidden bg-white w-20 h-20 shadow-xl p-0.5 mb-3">
      ...
      <div className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full mt-6">
        {!searchQuery && banners && banners.length > 0 && (
          <div className="w-full aspect-[21/9] sm:aspect-[4/1] mb-10 rounded-2xl overflow-hidden shadow-sm bg-gray-50 border border-gray-100">
            <img src={banners[currentBanner].image} className="w-full h-full object-contain" alt="banner" />
          </div>
        )}
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={() => setSelectedItem(null)}>
          <div className="bg-white rounded-[32px] overflow-hidden w-full max-w-sm shadow-2xl relative" onClick={e => e.stopPropagation()}>
            <button onClick={() => setSelectedItem(null)} className="absolute top-4 right-4 bg-black/40 text-white p-2 rounded-full z-10"><X size={20} /></button>
