...
import { getInventory, adjustStockAction, createIngredient } from "@/lib/actions";

type AdjustmentReason = "Restock" | "Sold" | "Waste" | "Manual";

export default function InventoryManager({ userName = "Admin" }: { userName?: string }) {
  ...
  const lowStockItems = ingredients.filter(ing => (ing.current / ing.max) * 100 < ing.lowThreshold);

  const displayedIngredients = ingredients.filter((item) => {
    const matchesSearch = item.name.toLowerCase().includes(cardSearch.toLowerCase());
    const pct = (item.current / item.max) * 100;
    const matchesFilter = cardFilter === "All" ? true : pct < item.lowThreshold;
    return matchesSearch && matchesFilter;
  });

  const filteredLog = stockLog.filter((entry) => logFilter === "All" ? true : entry.reason === logFilter);
  const paginatedLog = filteredLog.slice(0, logVisibleCount);
  ...
  const handleAdjustSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedItem || amount === "" || Number(amount) <= 0) return;
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newItemName || !newItemUnit || newItemMax === "" || newItemThreshold === "") return;

    startTransition(async () => {
      const res = await createIngredient({
        name: newItemName,
        unit: newItemUnit,
        max: Number(newItemMax),
        lowThreshold: Number(newItemThreshold)
      });
  ...
  if (isLoading) {
    return <div className="flex justify-center items-center py-20"><Loader2 className="animate-spin text-gray-400" size={32} /></div>;
  }
    ...
    <div className="w-full pb-12 font-sans text-gray-800 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="flex items-center gap-1.5 bg-gray-900 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-gray-800 transition-all active:scale-95 shadow-sm"
            >
      ...
      <div>
        ...
        <div className="bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-sm flex flex-col">
          ...
          {filteredLog.length > paginatedLog.length && (
            <div className="p-4 bg-gray-50/50 border-t border-gray-100 flex justify-center">
              <button 
                onClick={() => setLogVisibleCount(prev => prev + 10)}
                className="flex items-center gap-2 px-6 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-700 hover:text-gray-900 hover:bg-gray-50 transition-colors shadow-sm active:scale-95"
              >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-sm rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleAdjustSubmit} className="p-6">
              ...
              <button type="submit" disabled={isPending} className="w-full bg-gray-900 hover:bg-gray-800 text-white font-bold py-4 rounded-xl shadow-md transition-all active:scale-[0.98] text-[15px] disabled:opacity-50">
                {isPending ? <Loader2 className="animate-spin mx-auto" size={20} /> : `Confirm ${adjustmentMode === "restock" ? "Restock" : "Adjustment"}`}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-sm rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleCreateSubmit} className="p-6">
              ...
              <button type="submit" disabled={isPending} className="w-full bg-gray-900 hover:bg-gray-800 text-white font-bold py-4 rounded-xl shadow-md transition-all active:scale-[0.98] text-[15px] disabled:opacity-50">
                {isPending ? <Loader2 className="animate-spin mx-auto" size={20} /> : "Create Item"}
              </button>
            </form>
          </div>
        </div>
